...
//            pnlThongTinHD.add(lbDate, gbc);
        ...
        JButton btnAddHD = new JButton("THANH TOÁN");
...
//                hd.setNgayLap(new java.sql.Date(new Date().getTime()));
                ...
                timestamp.setTime(timestamp.getTime() + timeInMillis);

                // Convert java.sql.Timestamp back to java.sql.Date
                sqlDate = new java.sql.Date(timestamp.getTime());
                ///////
                ...
                bus.addHD(hd);
                ArrayList<product_DTO> tmp = new ArrayList<>();
                for (int i = 0; i < selectedSP.size(); i++){
                    ChiTietHD chiTietHD = new ChiTietHD();
                    chiTietHD.setMAMH(selectedSP.get(i).getMaSP());
                    ...
                    selectedSP.get(i).setSoLuong(- Integer.parseInt( (String) tblHD.getValueAt(i, 3)));
//                    System.out.println(selectedSP.get(i).getSoLuong() - Integer.parseInt( (String) tblHD.getValueAt(i, 3)));
                    tmp.add(selectedSP.get(i));
                }
                new product_DAO().addOldProduct(tmp);
                updateAfterAdd();
//                ;
                Desktop desktop = Desktop.getDesktop();
                try {
                    desktop.open(new File(ExportHoaDonToPDF.printPDF(hd)));
                } catch (IOException ex) {
                    throw new RuntimeException(ex);
                }
//                new PdfViewerDialog(MainMenu.currentFrame, ExportHoaDonToPDF.printPDF(hd)).setVisible(true);
            }
